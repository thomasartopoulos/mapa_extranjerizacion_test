...
def sanear(texto_csv):
    ...
    headers = [normalizar(h) for h in filas[0]]

    sensibles = [filas[0][i] for i, h in enumerate(headers) if h in SHEET_COLS_EXCLUIDAS]
    if sensibles:
    ...
    if 'provincia' not in idx:
        raise SystemExit(f"No encontré la columna Provincia. Encabezados: {filas[0]}")
